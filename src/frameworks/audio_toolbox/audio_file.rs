...
fn ExtAudioFileRead(
    ...
    'outer: for packet in data[..actually_read].chunks(packet_size as usize) {
        ...
        loop {
            ...
            if ((buf_offset + decode_ima4(/* &[u8; 34] */).len() as GuestUSize) * 2) < buf.data_byte_size {
                let target = env
                    .mem
                    .ptr_at_mut(buf.data.cast::<i16>() + buf_offset, decode_ima4(/* &[u8; 34] */).len() as GuestUSize,
                unsafe {
                    slice::from_raw_parts_mut(target, decode_ima4(/* &[u8; 34] */).len()).copy_from_slice(&pcm);
                }
                packets_consumed += 1;
                break;
            }
